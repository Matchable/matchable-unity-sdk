...
namespace HTTP
{
    ...
    public class Request
    {
        ...
        /// <summary>
        /// Converts a ArrayList object into a JSON string
        /// </summary>
        /// <param name="list">A ArrayList of  serializable object</param>
        /// <returns>A JSON encoded string, or "[]" if objects in the list are not serializable</returns>
        public static string JsonEncode(ArrayList list)
        {
            string[] result = new string[list.Count];
            for (int i = 0; i < result.Length; i++)
            {
                result[i] = JsonUtility.ToJson(list[i]);
            }
        ...
        public Request(string method, string uri, object data)
        {
            this.method = method;
            this.uri = new Uri(uri);
            this.byteStream = new MemoryStream(Encoding.UTF8.GetBytes(JsonUtility.ToJson(data)));
            this.AddHeader("Content-Type", "application/json");
        }

        public Request(string method, string uri, ArrayList data)
        {
            this.method = method;
            this.uri = new Uri(uri);
            this.byteStream = new MemoryStream(Encoding.UTF8.GetBytes(JsonEncode(data)));
            this.AddHeader("Content-Type", "application/json");
        }
